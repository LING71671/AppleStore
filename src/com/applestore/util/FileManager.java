...
public class FileManager {
    ...
    public static boolean saveProducts(ProductManager productManager) {
        ...
        try (ObjectOutputStream oos = new ObjectOutputStream(
            ...
            ColorPrinter.printSuccess("产品数据已成功保存到文件");
            return true;
            
        } catch (IOException e) {
            ColorPrinter.printError("保存产品数据失败: " + e.getMessage());
            return false;
        }
    }
    ...
    public static List<Product> loadProducts() {
        ...
        if (!file.exists()) {
            ColorPrinter.printInfo("数据文件不存在，将创建新文件");
        ...
        try (ObjectInputStream ois = new ObjectInputStream(
            ...
            ColorPrinter.printSuccess("成功从文件加载 " + products.size() + " 个产品");
            return products;
            
        } catch (IOException | ClassNotFoundException e) {
            ColorPrinter.printError("加载产品数据失败: " + e.getMessage());
            return new ArrayList<>();
        }
    }
    ...
    public static boolean exportToCSV(ProductManager productManager, String filename) {
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(DATA_DIR + "/" + filename))) {
            ...
            writer.println("ID,产品名称,型号,颜色,存储(GB),价格,库存");
            ...
            ColorPrinter.printSuccess("数据已导出到 " + filename);
            return true;
            
        } catch (IOException e) {
            ColorPrinter.printError("导出CSV失败: " + e.getMessage());
            return false;
        }
    }
    ...
    public static boolean importFromCSV(String filename, ProductManager productManager) {
        ...
        if (!file.exists()) {
            ColorPrinter.printError("文件不存在: " + filename);
            return false;
        }
            ...
            while (scanner.hasNextLine()) {
                ...
                if (data.length >= 7) {
                    try {
                        ...
                        ColorPrinter.printWarning("跳过无效数据行: " + line);
                    }
                }
            }
            
            ColorPrinter.printSuccess("成功导入 " + count + " 个产品");
            return true;
            
        } catch (IOException e) {
            ColorPrinter.printError("导入CSV失败: " + e.getMessage());
            return false;
        }
    }
    ...
    public static void clearDataFile() {
        ...
        if (file.exists()) {
            file.delete();
            ColorPrinter.printInfo("数据文件已清空");
        }
    }
}
